...
describe('BUG-076: Memory limit enforcement', () => {
  it('should enforce memory limits and emit pressure events', (done) => {
...
for (let i = 0; i < 50; i++) {
  ...
  mockAdapter.emit('changeGroup:changes', {
    ...
    setTimeout(() => {
      ...
      expect(memoryPressureLevel).toBeDefined();
      expect(stats.totalUsage).toBeLessThanOrEqual(stats.limit);
