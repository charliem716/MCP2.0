...
process.stdout.write = function(chunk, encoding, callback) {
  ...
  if (str.includes('Disconnecting from Q-SYS Core') || str.includes('Disconnected successfully')) {
    logCount++;
  }
...
async function demonstrateFix() {
  ...
  console.log(`- Result: ${logCount <= 2 ? '✅ PASS - No excessive logging!' : '❌ FAIL - Excessive logging detected'}`);
  ...
  if (flagKey) {
    ...
    console.log(`- Result: ${!client[flagKey] ? '✅ PASS - Flag properly reset!' : '❌ FAIL - Flag not reset'}`);
  }
