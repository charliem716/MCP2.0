...
const testCases = [
  ...
  {
    input: 'Component.channel.1.gain',
    expected: { comp: 'Component', ctrl: 'channel.1.gain' },
  },
];
...
testCases.forEach(test => {
  ...
  const passed =
    compName === test.expected.comp && ctrlName === test.expected.ctrl;
  console.log(
    `${passed ? '✅' : '❌'} ${test.input} → comp: "${compName}", ctrl: "${ctrlName}"`
  );

  if (!passed) {
    console.log(
      `   Expected: comp: "${test.expected.comp}", ctrl: "${test.expected.ctrl}"`
    );
  }
});
