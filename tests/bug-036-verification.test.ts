  ...
  it('should execute the test scenario', async () => {
    ...
    async function verifyComponentFormat() {
      try {
        ...
        // Connect to Q-SYS
        // console.log(
          // 'Connecting to Q-SYS Core at',
          // `${config.qsysCore.host}:${config.qsysCore.port || 443}`
        // );
        ...
        for (let i = 0; i < displayCount; i++) {
          ...
          // console.log(
          //   `  - Type field: ${comp.Type ? `✓ Present (${comp.Type})` : '✗ Missing'}`
          // );
          // console.log(
          //   `  - Properties field: ${comp.Properties ? `✓ Present (${comp.Properties.length} items)` : '✗ Missing'}`
          // );
    
          // Check if Type is specific or generic
          if (comp.Type === 'Component' || comp.Type === 'generic') {
            // console.log(
              // '  ⚠ Type appears to be generic, not specific component type'
            // );
          }
          ...
          if (comp.Properties && comp.Properties.length > 0) {
            ...
            // console.log(
              // `    - Name field: ${firstProp.Name ? '✓ Present' : '✗ Missing'}`
            // );
            // console.log(
            //   `    - Value field: ${firstProp.Value !== undefined ? '✓ Present' : '✗ Missing'}`
            // );
          }
        ...
        for (const comp of components) {
          ...
          const isGenericType =
            comp.Type === 'Component' || comp.Type === 'generic';
        ...
        if (compliantCount === components.length && genericTypeCount === 0) {
          // console.log(
            // '✓ PASS: All components match Q-SYS API specification exactly!'
          // );
        } else {
          ...
          if (missingPropertiesCount > 0) {
            // console.log(
              // `  - ${missingPropertiesCount} components missing Properties array`
            // );
          }
          if (genericTypeCount > 0) {
            // console.log(
              // `  - ${genericTypeCount} components have generic Type instead of specific type`
            // );
          }
          if (missingTypeCount > 0) {
            // console.log(
              // `  - ${missingTypeCount} components missing Type field entirely`
            // );
          }
        }
