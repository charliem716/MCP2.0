...
describe('BUG-076: Warning Levels Test', () => {
  it('should emit correct warning levels', async () => {
    ...
    console.log('\nWarnings received:');
    warnings.forEach(w =>
      console.log(`- ${w.level} at ${w.percentage.toFixed(2)}%`)
    );

    const has80Warning = warnings.some(
      w => w.level === 'high' && w.percentage >= 80 && w.percentage < 90
    );
    const has90Warning = warnings.some(
      w => w.level === 'critical' && w.percentage >= 90
    );
