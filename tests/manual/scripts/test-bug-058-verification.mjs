...
async function testBug058Fix() {
  ...
  try {
    ...
    const checks = [
      {
        name: 'Mode parameter schema',
        pattern: /mode.*summary.*filtered.*full/,
        found: false,
      },
      {
        name: 'Summary mode implementation',
        pattern: /generateSummaryResponse/,
        found: false,
      },
      {
        name: 'Filter validation',
        pattern: /Filter required when using.*filtered.*mode/,
        found: false,
      },
