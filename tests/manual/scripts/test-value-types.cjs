    ...
    for (const line of lines) {
      ...
      if (msg.id === 1 && msg.result) {
        ...
        controls.forEach(c => {
          ...
          if (isObject) {
            console.log(`Object value found in ${c.name}:`, JSON.stringify(c.value));
          }
        });
        ...
        const examples = {};
        controls.forEach(c => {
          const type = c.value && typeof c.value === 'object' ? 'object' : typeof c.value;
          if (!examples[type]) {
            examples[type] = { name: c.name, value: c.value, controlType: c.type };
          }
        });
        
        Object.entries(examples).forEach(([type, ex]) => {
          console.log(`  ${type}: ${ex.name} = ${JSON.stringify(ex.value)} (${ex.controlType})`);
        });
