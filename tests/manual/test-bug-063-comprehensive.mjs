...
async function testNormalShutdown() {
  ...
  return new Promise((resolve) => {
    ...
    proc.on('exit', (code) => {
      ...
      const hasStateCheck = logs.some(l => l.includes('State persistence check'));
...
async function testModuleCleanup() {
  ...
  return new Promise((resolve) => {
    ...
    proc.stderr.on('data', (data) => {
      ...
      if (line.includes('MCP server shut down')) cleanupSequence.push('mcp-shutdown');
    });
    ...
    proc.on('exit', () => {
      ...
      console.log(`  - Main cleanup: ${cleanupSequence.includes('main-cleanup') ? '✅' : '❌'}`);
      console.log(`  - QRWC disconnect: ${cleanupSequence.includes('qrwc-disconnect') ? '✅' : '❌'}`);
      console.log(`  - State check: ${cleanupSequence.includes('state-check') ? '✅' : '❌'}`);
      console.log(`  - MCP shutdown: ${cleanupSequence.includes('mcp-shutdown') ? '✅' : '❌'}`);
      
      const correctOrder = cleanupSequence.includes('main-cleanup') && 
