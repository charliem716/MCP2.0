...
function sendCommand(method, params) {
  return new Promise((resolve, reject) => {
    ...
    const handler = (data) => {
      ...
      if (response.id === id) {
        console.log(`📥 Response received (${response.result ? 'success' : 'error'})`);
      ...
      try {
        ...
        if (controls && controls.length > 0) {
          controls.slice(0, 5).forEach((ctrl, idx) => {
            ...
            console.log(`     Type: ${ctrl.Type}, Value: ${ctrl.Value}, Direction: ${ctrl.Direction}`);
          });
          ...
          const muteControl = controls.find(c => c.Name.toLowerCase().includes('mute') && c.Direction !== 'Read');
          const gainControl = controls.find(c => (c.Name.toLowerCase().includes('gain') || c.Name.toLowerCase().includes('level')) && c.Direction !== 'Read');
          
          if (muteControl) {
            console.log(`\n  📍 Found mute control: "${muteControl.Name}" (current: ${muteControl.Value})`);
          }
          if (gainControl) {
            console.log(`  📍 Found gain control: "${gainControl.Name}" (current: ${gainControl.Value})`);
          }
        } else {
