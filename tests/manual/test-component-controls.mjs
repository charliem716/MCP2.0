...
socket.on('open', async () => {
  ...
  try {
    ...
    const gainComponents = components.filter(c => 
    ...
    if (gainComponents.length > 0) {
      ...
      const controls = Array.isArray(controlsResponse) ? controlsResponse : 
                      controlsResponse.Controls ? controlsResponse.Controls : [];
      
      console.log(`\nFound ${controls.length} controls:`);
      if (controls.length === 0) {
        console.log('  No controls found - component might not be scriptable');
        console.log('  Full response:', JSON.stringify(controlsResponse, null, 2));
      } else {
        controls.forEach(c => console.log(`  - ${c.Name}: ${c.Value} (${c.Type})`));
      }
