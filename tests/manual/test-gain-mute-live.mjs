...
async function testGainMute() {
  ...
  try {
    ...
    let controls;
    if (controlsResult.content && controlsResult.content[0] && controlsResult.content[0].text) {
      ...
      if (jsonMatch) {
        ...
        const directResponse = await adapter.sendCommand('Component.GetAllControls', {});
    ...
    controls.forEach(control => {
      ...
      if (componentName.toLowerCase().includes('gain') || 
    ...
    if (muteControls.length > 0) {
      ...
      // Parse MCP response
      let currentValues;
      if (currentResult.content && currentResult.content[0] && currentResult.content[0].text) {
      ...
      console.log(`   Current mute state: ${currentMuteState ? '🔇 MUTED' : '🔊 UNMUTED'}`);
      ...
      // Parse MCP response
      let verifyValues;
      if (verifyResult.content && verifyResult.content[0] && verifyResult.content[0].text) {
      ...
      console.log(`   New mute state: ${newMuteState ? '🔇 MUTED' : '🔊 UNMUTED'}`);
