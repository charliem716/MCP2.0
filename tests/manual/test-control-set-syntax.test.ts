...
socket.on('open', async () => {
  ...
  try {
    console.log(
      '1️⃣ Testing Control.Set command syntax (expecting error code 8 for non-existent control)...'
    );
    ...
    if (correctSyntax.error && correctSyntax.error.code === 8) {
      ...
      console.log(
        "   Q-SYS recognized the command but reported the control doesn't exist (expected)"
      );
    }
    ...
    if (incorrectCommand.error && incorrectCommand.error.code !== 8) {
      ...
      console.log(
        `   Error: ${incorrectCommand.error.message} (code: ${incorrectCommand.error.code})`
      );
    }
    ...
    if (incorrectParams.error) {
      ...
      console.log(
        `   Error: ${incorrectParams.error.message} (code: ${incorrectParams.error.code})`
      );
    }
