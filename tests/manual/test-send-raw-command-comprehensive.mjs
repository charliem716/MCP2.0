...
async function runComprehensiveTests() {
  console.log(`${colors.cyan}🧪 Comprehensive send_raw_command Test Suite${colors.reset}`);
  ...
  try {
    ...
    await runTest('1.1 NoOp Command', async () => {
      logTest('NoOp', 'Simplest command - no operation, just tests connectivity');
    ...
    await runTest('2.1 ComponentGet with Control Array', async () => {
      logTest('ComponentGet', 'Get specific controls from a component using array syntax');
    ...
    await runTest('2.2 Complex Nested Parameters', async () => {
      logTest('ComponentSet', 'Test complex parameter structure (read-only simulation)');
    ...
    await runTest('3.2 Invalid Parameters', async () => {
      ...
      logInfo(`Error: ${response.error?.message || 'Parameter validation failed'}`);
      return response;
    });
    ...
    await runTest('3.3 Blocked Command', async () => {
      ...
      try {
        ...
        if (result.isError && result.content?.[0]?.text?.includes('blocked for safety')) {
          errorOccurred = true;
          errorMessage = result.content[0].text;
        } else {
          // Try to parse response if not isError
          const response = parseResponse(result);
          if (response.success === false && response.error?.message?.includes('blocked for safety')) {
            errorOccurred = true;
    ...
    await runTest('4.1 Quick Command Performance', async () => {
      logTest('NoOp with Short Timeout', 'Test fast command with 1 second timeout');
    ...
    await runTest('4.2 Rapid Sequential Commands', async () => {
      ...
      logInfo(`Total time: ${totalDuration}ms, Average: ${avgDuration.toFixed(2)}ms`);
    ...
    await runTest('4.3 Maximum Timeout Test', async () => {
      logTest('StatusGet with Max Timeout', 'Test maximum allowed timeout (30 seconds)');
    ...
    console.log('Testing write commands with safety checks (using safe values)');
    ...
    await runTest('5.1 Safe Write Test (No-op Value Change)', async () => {
      ...
      const writeableControl = controls.find(c => 
        c.value?.Direction === 'Read/Write' && 
        c.type === 'gain'
      );
    ...
    await runTest('5.2 Ramp Parameter Test', async () => {
      logTest('ComponentSet with Ramp', 'Test ramp parameter structure (validation only)');
    ...
    await runTest('5.3 Warning Command Test', async () => {
      logTest('Logon Command', 'Test command that triggers warning (no actual login)');
    ...
    testResults.scenarios.forEach(scenario => {
      ...
      const detail = scenario.status === 'PASS' 
