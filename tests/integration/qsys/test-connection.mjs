...
console.log(
  `🔐 Password: "${password ? '***' : ''}" ${password ? '(with auth)' : '(no auth)'}`
);
console.log('='.repeat(50));
...
socket.on('open', async () => {
  ...
  try {
    ...
    if (componentNames.length > 0) {
      ...
      console.log(
        '⚠️  No scriptable components found. Make sure components are marked as scriptable in Q-SYS Designer.'
      );
    }
...
socket.on('error', error => {
  ...
  if (error.code === 'ECONNREFUSED') {
    ...
    console.error(
      '   • Ensure "Allow External Control" is enabled in Q-SYS Designer'
    );
