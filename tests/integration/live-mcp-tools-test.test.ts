...
async function loadModules() {
  try {
    ...
    const adapterModule = await import('../../src/mcp/qrwc/adapter');
    const handlersModule = await import('../../src/mcp/handlers/index');
    const repositoryModule = await import('../../src/mcp/state/repository');
  ...
  try {
    ...
    await runTest('list_components - With filter', async () => {
      ...
      if (components.length > 0) {
        ...
        if (components[0].Properties) {
          console.log(
            `Properties: ${JSON.stringify(components[0].Properties, null, 2)}`
          );
        }
      }
    });
    ...
    await runTest('list_controls - All types', async () => {
      ...
      if (sampleControl) {
        ...
        console.log(
          `Metadata: ${JSON.stringify(sampleControl.metadata, null, 2)}`
        );
      }
    });
    ...
    await runTest('list_controls - Gain controls only', async () => {
      ...
      if (controls.length > 0) {
        ...
        controls.slice(0, 3).forEach(ctrl => {
          console.log(
            `  - ${ctrl.name} (Component: ${ctrl.component || 'N/A'})`
          );
        });
      }
    });
    ...
    await runTest('query_core_status', async () => {
      ...
      if (status.Status.Code !== 0)
        throw new Error(`Core status code: ${status.Status.Code}`);
    });
