...
async function runAllTests() {
  ...
  try {
    ...
    await runTest('Tool: query_core_status', async () => {
      ...
      console.log(
        `  Status: ${status.Status?.Name || 'Unknown'} (Code: ${status.Status?.Code || 'N/A'})`
      );
      ...
      if (status.Status?.Code && status.Status.Code !== 0) {
        console.warn(
          `⚠️  Core has non-zero status code: ${status.Status.Code}`
        );
      }
    });
    ...
    await runTest('Tool: get_control_values', async () => {
      ...
      values.forEach(val => {
        const position =
          val.position !== undefined ? val.position.toFixed(2) : 'N/A';
    ...
    await runTest('Tool: set_control_values (with ramp)', async () => {
      ...
      console.log(
        `Result: ${typeof setResponse === 'string' ? setResponse : 'Success'}`
      );
