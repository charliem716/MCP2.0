...
testCases.forEach(test => {
  ...
  const passed = result.compName === test.expected.compName && 
                 result.ctrlName === test.expected.ctrlName;
  
  console.log(`Test: ${test.description}`);
  console.log(`Input: "${test.input}"`);
  console.log(`Expected: component="${test.expected.compName}", control="${test.expected.ctrlName}"`);
  console.log(`Got:      component="${result.compName}", control="${result.ctrlName}"`);
