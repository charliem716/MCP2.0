...
async function runTests() {
  console.log('Test 1: Verify no excessive logging on multiple disconnect calls');
  ...
  const disconnectLogs = capturedLogs.filter(log => 
    log.message.includes('Disconnecting') || log.message.includes('Disconnected')
  );
  ...
  console.log(`- Flag before disconnect: ${flagBeforeDisconnect} (expected: false)`);
  
  // During first call (need to check inside disconnect method)
  // After disconnect
  client2.disconnect();
  const flagAfterDisconnect = client2.shutdownInProgress;
  console.log(`- Flag after disconnect: ${flagAfterDisconnect} (expected: false)`);
