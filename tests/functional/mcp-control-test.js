...
async function runTests() {
  ...
  try {
    ...
    await tester.runTest('Create Safety Snapshot', async () => {
      ...
      const snapshotComp = Array.from(tester.testComponents.values())
        .find(c => c.type.toLowerCase().includes('snapshot'));
      ...
      if (snapshotComp) {
        ...
        if (response.error) {
          throw new Error(`Failed to create snapshot: ${response.error.message}`);
        }
        
        console.log(`   Snapshot saved: ${tester.snapshotName}`);
      } else {
        console.log('   ⚠️  No snapshot component found - proceeding without safety snapshot');
      }
    });
    ...
    await tester.runTest('Get All Controls with Pagination', async () => {
      ...
      console.log(`   Page 1 of ${result.total_pages} (${result.controls.length} controls)`);
    });

    // 5. CONTROL VALUE OPERATIONS
    await tester.runTest('Get Control Values', async () => {
      // Find a gain control to test
      const gainControl = Array.from(tester.testComponents.values())
        .find(c => c.type.toLowerCase().includes('gain'));
    ...
    await tester.runTest('Set Control Value with Ramp', async () => {
      ...
      if (response.error) {
        throw new Error(`Failed to set control value: ${response.error.message}`);
      }
      
      console.log(`   Set ${gainControl.name}.gain to ${testValue} dB with 1s ramp`);
      ...
      if (Math.abs(newValues[0].value - testValue) > 0.1) {
        throw new Error(`Value not set correctly. Expected ${testValue}, got ${newValues[0].value}`);
      }
    ...
    await tester.runTest('Get Multiple Controls from Component', async () => {
      ...
      const component = Array.from(tester.testComponents.values())
        .find(c => c.type.toLowerCase().includes('mixer') || 
      ...
      console.log(`   Retrieved ${controls.length} controls from ${component.name}`);
    ...
    await tester.runTest('Find Non-Default Values', async () => {
      ...
      console.log(`   Found ${result.total_controls} controls with non-default values`);
    ...
    await tester.runTest('Restore Safety Snapshot', async () => {
      ...
      if (snapshotComp) {
        ...
        if (response.error) {
          throw new Error(`Failed to restore snapshot: ${response.error.message}`);
        }
        
        console.log(`   System restored to pre-test state`);
      } else {
        console.log('   ⚠️  No snapshot component - manual restoration may be needed');
      }
    });
