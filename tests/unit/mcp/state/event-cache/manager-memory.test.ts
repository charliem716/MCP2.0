...
describe('EventCacheManager - Memory Management', () => {
  ...
  describe('Memory Usage Calculation', () => {
    it('should calculate memory usage across all buffers', () => {
      ...
      const event2 = {
        groupId: 'group2',
        changes: [{ Name: 'control3', Value: true, String: 'true' }],
        timestamp: BigInt(Date.now()) * 1000000n,
        timestampMs: Date.now(),
        sequenceNumber: 2,
      };
  ...
  describe.skip('Memory Pressure Handling', () => {
    ...
    it('should respect group priorities during eviction', () => {
      ...
      manager.on('memoryPressureResolved', () => {
        const stats = manager.getMemoryStats();
        const groupStats = stats.groupStats.reduce(
          (acc, stat) => {
        ...
        expect(groupStats['important']).toBeGreaterThan(
          groupStats['unimportant']
        );
        done();
      });
