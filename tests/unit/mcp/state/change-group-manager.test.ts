...
describe('ChangeGroupManager', () => {
  ...
  describe('executeChangeGroup', () => {
    it('should execute change group successfully', async () => {
      ...
      expect(mockQrwcClient.sendCommand).toHaveBeenCalledWith('Control.GetValues', { Names: ['control0'] });
      expect(mockQrwcClient.sendCommand).toHaveBeenCalledWith('Control.SetRamp', {
        Name: 'control0',
        Value: 0,
        Ramp: 1.5
      });
      expect(mockQrwcClient.sendCommand).toHaveBeenCalledWith('Control.GetValues', { Names: ['control1'] });
      expect(mockQrwcClient.sendCommand).toHaveBeenCalledWith('Control.SetValues', {
    ...
    it('should handle partial failures with rollback', async () => {
      ...
      await expect(manager.executeChangeGroup(changeGroup, {
  ...
  describe('error handling', () => {
    ...
    it('should handle client errors gracefully', async () => {
      ...
      await expect(manager.executeChangeGroup(changeGroup)).rejects.toThrow('Q-SYS connection lost');
