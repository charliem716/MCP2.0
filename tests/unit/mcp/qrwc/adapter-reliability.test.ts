  ...
  describe('Performance Optimization', () => {
    describe('Request batching', () => {
      it('should batch multiple control sets within time window', async () => {
        ...
        for (let i = 0; i < 5; i++) {
          promises.push(adapter.sendCommand('Control.Set', {
            Name: `control${i}`,
            Value: i
          }));
        }
    ...
    describe('Connection pooling', () => {
      it('should reuse connections for multiple requests', async () => {
        ...
        mockClient.sendCommand.mockImplementation(() => {
          ...
          const connId = (adapter as any).connectionId || 
    ...
    describe('Memory management', () => {
      it('should limit cache memory usage', async () => {
        ...
        const createLargeComponent = (name: string) => ({
          Name: name,
          Controls: Array(100).fill(null).map((_, i) => ({
            Name: `control${i}`,
            Value: Math.random(),
            Properties: {
              MinValue: -100,
    ...
    describe('Request deduplication', () => {
      it('should deduplicate concurrent identical requests', async () => {
        ...
        const promises = Array(5).fill(null).map(() => 
          adapter.sendCommand('Component.Get', { Name: 'Test' })
        );
