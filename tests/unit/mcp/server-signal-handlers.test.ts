...
describe('MCPServer Event Listener Cleanup (BUG-028)', () => {
  ...
  beforeEach(() => {
    ...
    mockProcess.removeListener = jest.fn(EventEmitter.prototype.removeListener.bind(mockProcess));
    mockProcess.listenerCount = EventEmitter.prototype.listenerCount.bind(mockProcess);
  ...
  it('should accumulate signal handlers when multiple servers are created', async () => {
    ...
    const setupGracefulShutdown = (server1 as any).setupGracefulShutdown.bind(server1);
    ...
    const setupGracefulShutdown2 = (server2 as any).setupGracefulShutdown.bind(server2);
  ...
  it('should remove signal handlers on shutdown', async () => {
    ...
    const setupGracefulShutdown = (server as any).setupGracefulShutdown.bind(server);
    setupGracefulShutdown();
