...
describe('OfficialQRWCClient - Comprehensive Coverage Tests', () => {
  ...
  beforeEach(async () => {
    jest.resetModules();
    jest.clearAllMocks();
    jest.useRealTimers(); // Use real timers to avoid issues
  ...
  describe('Reconnection and Long-term Mode', () => {
    ...
    it('should handle reconnection with downtime tracking', async () => {
      const client = new OfficialQRWCClient({
        host: 'test.local',
        enableAutoReconnect: true,
        reconnectInterval: 50,
    ...
    it('should emit cache invalidation event for long downtime', async () => {
      const client = new OfficialQRWCClient({
        host: 'test.local',
        enableAutoReconnect: false,
      });
    ...
    it('should handle reconnection during shutdown', async () => {
      const client = new OfficialQRWCClient({
        host: 'test.local',
        enableAutoReconnect: true,
      });
