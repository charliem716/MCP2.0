...
function validateMemoryLimits(config: EventCacheConfig, errors: string[], warnings: string[]): void {
...
function validateEventLimits(config: EventCacheConfig, warnings: string[]): void {
...
function validateRetentionSettings(config: EventCacheConfig, warnings: string[]): void {
...
function validateSpilloverDirectory(directory: string, errors: string[], warnings: string[]): void {
  try {
    ...
    if (!fs.existsSync(directory)) {
      warnings.push(`Spillover directory will be created: ${directory}`);
    }
  } catch (error) {
...
function validateDiskSpillover(config: EventCacheConfig, errors: string[], warnings: string[]): void {
  const spilloverConfig = config.diskSpilloverConfig;
  if (!spilloverConfig) return;
  ...
  validateSpilloverDirectory(spilloverConfig.directory, errors, warnings);
...
function validateCompression(compressionConfig: NonNullable<EventCacheConfig['compressionConfig']>, errors: string[], warnings: string[]): void {
...
function validateMemoryCheckInterval(config: EventCacheConfig, errors: string[], warnings: string[]): void {
  if (config.memoryCheckIntervalMs !== undefined) {
    if (config.memoryCheckIntervalMs < 1000) {
    ...
    } else if (config.memoryCheckIntervalMs > 60000) {
      warnings.push('memoryCheckIntervalMs > 1min: Infrequent checks may delay memory management');
    }
  }
}

function validateCrossConfiguration(config: EventCacheConfig, warnings: string[]): void {
...
export function validateEventCacheConfig(config: EventCacheConfig): ValidationResult {
  ...
  validateMemoryLimits(config, errors, warnings);
  validateEventLimits(config, warnings);
  validateRetentionSettings(config, warnings);

  if (config.diskSpilloverConfig?.enabled) {
    validateDiskSpillover(config, errors, warnings);
  }

  if (config.compressionConfig?.enabled) {
    validateCompression(config.compressionConfig, errors, warnings);
  }

  validateMemoryCheckInterval(config, errors, warnings);
  validateCrossConfiguration(config, warnings);
  validatePerformanceImpact(config, warnings);
