...
describe('CircularBuffer', () => {
  describe('queryTimeRange bug reproduction', () => {
    it('should handle wraparound correctly when buffer is full', () => {
      ...
      console.log('Query results for 6-7:', results);
      ...
      console.log('Query results for 4-5:', olderResults);
      expect(olderResults).toEqual(['event4', 'event5']);
      
      // Query for events 1-3 (which should have been overwritten)
      const overwrittenResults = buffer.queryTimeRange(BigInt(1000), BigInt(3000));
      console.log('Query results for 1-3 (should be empty):', overwrittenResults);
    ...
    it('demonstrates the stale index problem', () => {
      ...
      console.log('Stale query result:', staleQuery);
    ...
    it('demonstrates the actual bug - performance issue with large buffers', () => {
      ...
      const buffer = new CircularBuffer<{ id: string; value: number }>(capacity);
      ...
      const queryResult = buffer.queryTimeRange(BigInt(9900000), BigInt(9999000));
      ...
      console.log(`getAll time: ${getAllTime / 1000000n}ms`);
      console.log(`Query returned ${queryResult.length} events`);
    ...
    it('shows the real bug - queryTimeRange returns wrong data after multiple overwrites', () => {
      ...
      console.log('Cycle1 query results:', cycle1Results);
      ...
      console.log('Cycle2 query results:', cycle2Results);
    ...
    it('demonstrates the actual performance bug - queryTimeRange is O(n) not O(log n)', () => {
      ...
      for (let i = 0; i < capacity / 2; i++) {
        buffer.add({ id: `new-event${i}`, value: i }, BigInt((capacity + i) * 1000));
      }
      ...
      console.log(`Query returned ${results.length} events in ${elapsed / 1000000n}ms`);
    ...
    it('FAILING TEST - demonstrates stale index bug after buffer wraparound', () => {
      ...
      console.log('Buffer contents:', buffer.getAll());
      ...
      console.log('Query for C and D:', cAndD);
      ...
      console.log('Query for overwritten B:', shouldBeEmpty);
