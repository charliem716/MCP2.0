...
export class CompressionEngine {
  ...
  compressEvents(
    ...
    // Mark compression as active if we actually compressed something
    if (compressed > 0) {
      this._isActive = true;
    }
  ...
  private isSignificantChange(
    ...
    if (
      ...
      typeof lastKeptEvent.value === 'number'
    ) {
      const percentChange =
        Math.abs((event.value - lastKeptEvent.value) / lastKeptEvent.value) *
        100;
