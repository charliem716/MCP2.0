...
describe('BUG-074 Fix: queryTimeRange performance', () => {
  it('should demonstrate O(log n) vs O(n) performance difference', () => {
    ...
    const buffer = new CircularBuffer<{ id: number; timestampMs: number }>(totalEvents);
    ...
    const filtered = allEvents.filter(e => 
      e.timestampMs >= 50000 && e.timestampMs <= 50100
    );
    const getAllTime = process.hrtime.bigint() - getAllStart;
    
    console.log(`queryTimeRange (O(log n)): ${queryTime / 1000000n}ms for ${queryResults.length} results`);
    console.log(`getAll + filter (O(n)): ${getAllTime / 1000000n}ms for ${filtered.length} results`);
  ...
  it('verifies queryTimeRange works correctly after fix', () => {
    const buffer = new CircularBuffer<{ control: string; value: number; timestampMs: number }>(1000);
