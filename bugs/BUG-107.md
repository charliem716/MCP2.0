...
## Additional Context for 100% Resolution

The core issue of `BUG-107` is not just that the tests are failing, but that the test *coverage* is too low. A passing test suite is meaningless if it's only testing a fraction of the codebase. To achieve 100% resolution, the following must be addressed:

1.  **Prioritize High-Impact Files:** The bug report identifies the files with the lowest test coverage. The developer should prioritize writing tests for these files, starting with `src/shared/utils/error-recovery.ts` (34.37%) and `src/qrwc/officialClient.ts` (40.67%).

2.  **Focus on Branch Coverage:** The largest coverage gap is in branch coverage (13.68% below the target). This means that `if/else` statements, ternary operators, and other conditional logic are not being tested. The developer should specifically write tests that exercise both the "if" and "else" paths of the conditional logic in the codebase.

3.  **Use a Coverage-Driven Workflow:** The developer should use the HTML coverage report generated by Jest (`coverage/lcov-report/index.html`) to visually identify the lines of code that are not being tested. They should then write tests that specifically target these uncovered lines.

4.  **Do Not Lower Thresholds:** While the bug report suggests lowering the coverage thresholds as a temporary workaround, this should be avoided. Lowering the thresholds simply hides the problem. The goal should be to meet the existing 80% threshold by writing more tests.
